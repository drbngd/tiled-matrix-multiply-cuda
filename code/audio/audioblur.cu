...
__device__ void cuda_blur_kernel_convolution(uint thread_index, const float* gpu_raw_data,
    ...
    float result = 0.0;
    if (thread_index < GAUSSIAN_SIZE) {
        for (int j = 0; j <=thread_index; j++) {
            result += gpu_raw_data[thread_index - j] * gpu_blur_v[j];
        }
    }
    if (thread_index >= GAUSSIAN_SIZE && thread_index < n_frames) {
        for (int j = 0; j < GAUSSIAN_SIZE; j++) {
            result += gpu_raw_data[thread_index - j] * gpu_blur_v[j];
        }
    }
    gpu_out_data[thread_index] = result;
...
__global__ void cuda_blur_kernel(const float *gpu_raw_data, const float *gpu_blur_v,
    ...
    extern __shared__ float sh_blur_v[];
...
//        sh_blur_v[threadIdx.x] = gpu_blur_v[threadIdx.x];
//    } else {
//        /* throw error */
...
//    }
    ...
    /* copy into shared mem */
    if (thread_index < blur_v_size) {
        sh_blur_v[thread_index] = gpu_blur_v[thread_index];
    }
    __syncthreads();
    ...
    while (thread_index <= n_frames) {
        ...
        cuda_blur_kernel_convolution(thread_index, gpu_raw_data,
                                     /*gpu_blur_v*/ sh_blur_v, gpu_out_data,
...
float cuda_call_blur_kernel(const unsigned int blocks,
    ...
    /* allocating sh_mem dynamically */
    cuda_blur_kernel<<<blocks, threads_per_block, blur_v_size * sizeof(float)>>>(gpu_raw_data, gpu_blur_v, gpu_out_data, n_frames, blur_v_size);
